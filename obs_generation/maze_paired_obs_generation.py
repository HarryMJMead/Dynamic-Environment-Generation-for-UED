...
def sample_trajectories(
    ...
    adv_init_traj = jax.tree.map(pad_traj, adv_init_traj)
    ...
    def sample_adv_step(carry):
        ...
        adv_full_traj = jax.tree.map(update_traj, adv_full_traj, traj_step)
        ...
        (next_hstate, next_obs, next_env_state, done, adv_traj_idxs) = jax.tree.map(
            replace_carry,
    ...
    def sample_step(carry, _):
        ...
        (adv_pro_rewards, adv_ant_rewards) = jax.tree.map(
...
def main(config=None, project="JAXUED_TEST"):
    ...
    run = wandb.init(config=wandb_config, project=project, tags=["PAIRED", "obs_gen",])
    ...
    def log_eval(stats):
        ...
        log_dict = {
            ...
            "misc/pro_kl_divergence":   stats['pro_losses'][-1][-1].mean(),
            "misc/ant_kl_divergence":   stats['ant_losses'][-1][-1].mean(),
        }
    ...
    def train_and_eval_step(runner_state, _):
        ...
        metrics["levels"] = jax.vmap(adv_env_renderer.render_level, (0, None))(jax.tree.map(lambda x: x[-1], metrics["levels"]), env_params)
