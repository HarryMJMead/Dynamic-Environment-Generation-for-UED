...
def main(config=None, project="JAXUED_TEST"):
    ...
    run = wandb.init(config=wandb_config, project=project, tags=["optimal_regret", "obs_gen",])
    ...
    def train_step(carry, _):
        ...
        # Assign Pro Regret to Adv (dense reward)
        rewards, _ = jax.lax.scan(
            add_regret_at_idx,
            jnp.zeros(rewards.shape),
            (agent_regret, adv_traj_idx_array),
        )

        # Assign Pro Regret to Adv (sparse reward)
        # rewards = jax.vmap(
        #     lambda x, i, y : jax.lax.dynamic_update_slice(x, y, (i-1,)),
        #     in_axes=(1, 0, 1)
        # )(jnp.zeros(rewards.shape, dtype=jnp.float32), adv_traj_idxs, agent_regret.sum(axis=0, keepdims=True)).swapaxes(0, 1)
