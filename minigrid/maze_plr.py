...
def main(config=None, project="JAXUED_TEST"):
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        ...
        def on_new_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            shortest_path = jax.vmap(get_shortest_path, in_axes=[0,])(new_levels)
            unsolvable = shortest_path == jnp.inf
            score_advantages, _ = compute_gae(config["gamma"], config["gae_lambda"], last_value, values, rewards, dones)
            ...
            sampler, _ = level_sampler.insert_batch(sampler, new_levels, scores, {"max_return": max_returns, "unsolvable": unsolvable})
        ...
        def on_replay_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            unsolvable = level_sampler.get_levels_extra(sampler, level_inds)["unsolvable"]
            scores = compute_score(config, dones, values, max_returns, score_advantages)
            sampler = level_sampler.update_batch(sampler, level_inds, scores, {"max_return": max_returns, "unsolvable": unsolvable})
        ...
        def on_mutate_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            shortest_path = jax.vmap(get_shortest_path, in_axes=[0,])(child_levels)
            unsolvable = shortest_path == jnp.inf
            score_advantages, _ = compute_gae(config["gamma"], config["gae_lambda"], last_value, values, rewards, dones)
            if config["score_function"] == "mna":
                score_advantages, _ = compute_clipped_gae(config[f"gamma"], config[f"gae_lambda"], last_value, values, rewards, dones, None, use_max_value=True)
            max_returns = compute_max_returns(dones, rewards)
            scores = compute_score(config, dones, values, max_returns, score_advantages)
            sampler, _ = level_sampler.insert_batch(sampler, child_levels, scores, {"max_return": max_returns, "unsolvable": unsolvable})
...
@hydra.main(config_path="config", config_name="main_plr", version_base=None)
