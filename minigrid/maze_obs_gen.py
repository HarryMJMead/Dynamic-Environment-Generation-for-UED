...
import wandb
from jaxued.environments.maze.env_editor import MazeEditor, Observation, ObservedMazeEditor, ObservedMazeEditorWithGoal, LocalMazeEditor, LocalMazeEditorRotate, LocalMazeEditorRotateSplitAct
...
def sample_trajectories(
    ...
    def sample_step(carry, _):
        ...
        def student_step(rng, train_state, carry, num_traj, take_step, not_done):
            ...
            location = jnp.concatenate([env_state.env_state.agent_dir[:, None], env_state.env_state.agent_pos], axis=-1)
            ...
            return rng, next_carry, location, not_done, (next_locs, next_dirs, next_goal_placed, next_value), traj
        rng, next_pro_carry, pro_location, pro_not_done, (next_locs, next_dirs, next_goal_placed, next_pro_value), pro_traj = student_step(rng, pro_train_state, pro_carry, num_pro_traj, take_pro_step, pro_not_done)
        ...
        #     take_adv_step[:, None],
        #     jnp.logical_and(next_adv_obs.action_mask, jnp.tile(jnp.concatenate([jnp.ones(adv_env.agent_view_size**2)]*2 + [jnp.zeros(adv_env.agent_view_size**2)]), adv_env.num_agents)),
        #     next_adv_obs.action_mask
        # )
        next_adv_carry = next_adv_hstate, next_adv_obs.replace(agent_values=next_pro_value, place_goal=jnp.logical_and(~take_adv_step, ~next_adv_env_state.level.goal_placed), goal_placed=next_goal_placed), next_adv_env_state, next_adv_last_done, next_adv_steps, next_adv_value
    ...
    last_pro_location = jnp.concatenate([last_pro_env_state.env_state.agent_dir[:, None], last_pro_env_state.env_state.agent_pos], axis=-1)
...
def sample_trajectories_rnn(
    ...
    x = jax.tree_map(lambda x: x[None, ...], (last_obs, last_done))
    _, _, last_value = train_state.apply_fn(train_state.params, x, hstate)
    last_location = jnp.concatenate([last_env_state.env_state.agent_dir[:, None], last_env_state.env_state.agent_pos], axis=-1)
...
class DoubleCategorical(distrax.Distribution):
    def __init__(self, logits_1, logits_2):
        ...
        target_probs = jnp.array([0.495, 0.495, 0.01])
...
class AdversaryActorCritic(nn.Module):
    ...
    def __call__(self, inputs: Tuple[Observation, chex.Array], hidden):
        ...
        actor_mean_1 = nn.Dense(3, kernel_init=orthogonal(0.01), bias_init=constant(0.0), name="actor11")(actor_mean)
...
def main(config=None, project="JAXUED_TEST"):
    ...
    def log_eval(stats):
        ...
        def make_caption(i):
            ...
            return f"P({pro_mean_returns:.2f}, {pro_extra_mean_returns:.2f})|R({pro_regret:.2f}, {pro_extra_regret:.2f})"
    ...
    adv_env = LocalMazeEditorRotateSplitAct(env, random_z_dimensions=config['adv_random_z_dimension'], zero_out_random_z=config['adv_zero_out_random_z'], num_agents=config["num_pro_traj"], agent_view_size=5)
    ...
    def create_train_state(rng):
        ...
        return TrainState(
            ...
            adv_train_state = create_inner_train_state(rng_adv, adv_env, adv_env_params, AdversaryActorCritic, "adv_", network_kws={"max_timesteps": config["adv_num_steps"], "student_max_timesteps": 250})
        )
    ...
    def train_step(carry, _):
        ...
        def rollout(rng, env, env_params, train_state, init_hstate, levels, num_steps, prefix):
            ...
            traj_idxs = jnp.ones(config['num_train_envs']) * config['student_num_steps']
            advantages, targets = compute_gae(config[f"{prefix}gamma"], config[f"{prefix}gae_lambda"], last_value, values, rewards, dones, traj_idxs)
            update_mask = jnp.tile(jnp.arange(actions.shape[0])[:, None], (1, actions.shape[1])) < traj_idxs
        ...
        agent_extra_optimal_distances = jax.vmap(
            get_agent_min_steps_to_goal,
            in_axes=(0, 1)
        )(adv_env_state_modified, locations.reshape(-1, config['num_train_envs'], 1, 3)).swapaxes(0, 1).squeeze(-1)
        ...
        def get_trajectory_metrics(traj, carry, last_value, locations, num_traj):
            ...
            agent_optimal_distances = jax.vmap(
                get_agent_min_steps_to_goal,
                in_axes=(0, 1)
            )(adv_carry[2], locations.reshape(-1, config['num_train_envs'], num_traj, 3)).swapaxes(0, 1)
