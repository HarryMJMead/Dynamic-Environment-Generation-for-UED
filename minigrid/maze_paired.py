...
def main(config=None, project="JAXUED_TEST"):
    ...
    group = "PAIRED" if config['set_init_pos'] else "PAIRED_Rand"
    ...
    def log_eval(stats):
        ...
        log_dict = {
            ...
            "misc/ant_regret":   stats['ant_regret'].mean(),
        }
        ...
        def make_caption(i):
            ...
            ant_regret       = jnp.round(stats['ant_regret'][-1][i], 2) # .flatten()
            return f"P({pro_mean_returns:.2f}, {pro_regret:.2f})|A({ant_max_returns:.2f}, {ant_regret:.2f})|R({est_regret:.2f})"
    ...
    adv_env = MazeEditor(env, random_z_dimensions=config['adv_random_z_dimension'], zero_out_random_z=config['adv_zero_out_random_z'], set_init_pos=config['set_init_pos'])
    eval_env = env
    ...
    def train_step(carry, _):
        ...
        def get_shortest_path(level):
            all_optimal_distances = compute_min_steps_to_goal(level)
            agent_pos = level.agent_pos
            agent_dir = level.agent_dir

            return all_optimal_distances[agent_dir, agent_pos[1], agent_pos[0]]
        ...
        adv_rollout, (_, _, last_env_state, last_value, _) = rollout(rng_rollout, adv_env, adv_env_params, adv_train_state, AdversaryActorCritic.initialize_carry((config["num_train_envs"],)), empty_levels, config["adv_num_steps"], "adv_")
        ...
        shortest_path = jax.vmap(get_shortest_path, in_axes=[0,])(levels)
        pro_traj_length = jnp.minimum(((1 - pro_mean_returns)*250/0.9), 250)
        ant_traj_length = jnp.minimum(((1 - ant_max_returns)*250/0.9), 250)

        pro_regret = jnp.maximum(pro_traj_length - shortest_path, 0)
        ant_regret = jnp.maximum(ant_traj_length - shortest_path, 0)
