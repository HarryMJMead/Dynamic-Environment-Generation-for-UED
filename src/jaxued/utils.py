...
def accumulate_rollout_stats_w_idxs(dones, metrics, *, time_average, idxs):
    def iter(carry, input):
        ...
        new_max = jnp.logical_and(done, val > max_val)
        max_idx = jnp.where(new_max, idx, max_idx)
        ...
        return (sum_val, max_val, accum_val, step_count, episode_count, max_idx), None
    
    batch_size = dones.shape[1]
    zeros = jax.tree_util.tree_map(lambda x: jnp.zeros_like(x[0]), metrics)
    (sum_val, max_val, _, _, episode_count, max_idxs), _ = jax.lax.scan(
        iter,
        (zeros, zeros, zeros, jnp.zeros(batch_size, dtype=jnp.uint32), jnp.zeros(batch_size, dtype=jnp.uint32), zeros),
        (dones, metrics, idxs),
    )
    
    mean_val = jax.tree_util.tree_map(lambda x: x / jnp.maximum(episode_count, 1), sum_val)
    
    return mean_val, max_val, episode_count, max_idxs

def compute_max_mean_returns_epcount_w_idxs(dones, rewards, idxs):
    return accumulate_rollout_stats_w_idxs(dones, rewards, time_average=False, idxs=idxs)