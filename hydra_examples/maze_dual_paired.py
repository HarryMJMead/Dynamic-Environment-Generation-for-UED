...
class TrainState:
    ...
    pro_adv_train_state: BaseTrainState
    ant_adv_train_state: BaseTrainState
...
def main(config=None, project="JAXUED_TEST"):
    ...
    run = wandb.init(config=wandb_config, project=project, tags=["PAIRED", "Dual",])
    ...
    def log_eval(stats):
        ...
        # generic stats
        env_steps = 2 * config["eval_freq"] * config["num_train_envs"] * config["student_num_steps"]
        log_dict = {
            ...
            "misc/pro_regret":   stats['pro_est_regret'].mean(),
            "misc/ant_regret":   stats['ant_est_regret'].mean(),
        }
        ...
        def make_caption(i):
            if i < config["num_train_envs"] // 2:
                pref_1, pref_2, = "pro", "ant"
                regret_i = i
            else:
                pref_1, pref_2 = "ant", "pro"
                regret_i = i - config["num_train_envs"] // 2
            pro_mean_returns = jnp.round(stats[f'{pref_1}_mean_returns'][-1][i], 2) # .flatten()
            ant_max_returns  = jnp.round(stats[f'{pref_2}_max_returns'][-1][i], 2) # .flatten()
            est_regret       = jnp.round(stats[f'{pref_1}_est_regret'][-1][regret_i], 2) # .flatten()
            return f"{pref_1[0].capitalize()}({pro_mean_returns:.2f})|{pref_2[0].capitalize()}({ant_max_returns:.2f})|R({est_regret:.2f})"
    ...
    def create_train_state(rng):
        def create_inner_train_state(rng, env, env_params, network_cls, prefix, num_envs, network_kws={}):
            ...
            obs = jax.tree.map(
                lambda x: jnp.repeat(jnp.repeat(x[None, ...], num_envs, axis=0)[None, ...], 256, axis=0),
                obs,
            )
            init_x = (obs, jnp.zeros((256, num_envs)))
            network = network_cls(env.action_space(env_params).n, **network_kws)
            network_params = network.init(rng, init_x, network_cls.initialize_carry((num_envs,)))
            tx = optax.chain(
        ...
        rng_pro, rng_ant, rng_pro_adv, rng_ant_adv = jax.random.split(rng, 4)
        return TrainState(
            update_count = 0,
            pro_train_state = create_inner_train_state(rng_pro, env, env_params, ActorCritic, "student_", config["num_train_envs"]),
            ant_train_state = create_inner_train_state(rng_ant, env, env_params, ActorCritic, "student_", config["num_train_envs"]),
            pro_adv_train_state = create_inner_train_state(rng_pro_adv, adv_env, adv_env_params, AdversaryActorCritic, "adv_", config["num_train_envs"] // 2, network_kws={"max_timesteps": config["adv_num_steps"]}),
            ant_adv_train_state = create_inner_train_state(rng_ant_adv, adv_env, adv_env_params, AdversaryActorCritic, "adv_", config["num_train_envs"] // 2, network_kws={"max_timesteps": config["adv_num_steps"]})
        )

    def train_step(carry, _):
        def concatenate_levels(levels_1, levels_2):
            return jnp.concatenate([levels_1, levels_2])

        def rollout(rng, env, env_params, train_state, init_hstate, levels, num_steps, prefix, num_envs):
            # Single rollout
        ...
        def compute_adv_advantage(adv_rollout, est_regret, last_value, obs_densities):
            ...
            mapped_densities /= mapped_densities.mean(axis=0)
            ...
            return (obs, actions, dones.at[-1].set(True), log_probs, values, targets, scaled_advantages)
        ...
        pro_adv_train_state = train_state.pro_adv_train_state
        ant_adv_train_state = train_state.ant_adv_train_state

        rng, _pa_rng, _aa_rng = jax.random.split(rng, 3)
        empty_levels = jax.tree.map(lambda x: jnp.array([x]).repeat(config["num_train_envs"]//2, axis=0), sample_empty_level())
        pro_adv_rollout, (_, _, pro_adv_last_env_state, pro_adv_last_value, _) = rollout(_pa_rng, adv_env, adv_env_params, pro_adv_train_state, AdversaryActorCritic.initialize_carry((config["num_train_envs"]//2,)), empty_levels, config["adv_num_steps"], "adv_", config["num_train_envs"]//2)
        ant_adv_rollout, (_, _, ant_adv_last_env_state, ant_adv_last_value, _) = rollout(_aa_rng, adv_env, adv_env_params, ant_adv_train_state, AdversaryActorCritic.initialize_carry((config["num_train_envs"]//2,)), empty_levels, config["adv_num_steps"], "adv_", config["num_train_envs"]//2)
        levels = jax.tree.map(concatenate_levels, pro_adv_last_env_state.level, ant_adv_last_env_state.level)

        # protagonist rollout
        rng, _rng = jax.random.split(rng)
        pro_rollout, (dones, rewards, _, _, _) = rollout(_rng, env, env_params, pro_train_state, ActorCritic.initialize_carry((config["num_train_envs"],)), levels, config["student_num_steps"], "student_", config["num_train_envs"])
        pro_mean_returns, pro_max_returns, pro_eps = compute_max_mean_returns_epcount(dones, rewards)

        # antagonist rollout
        rng, _rng = jax.random.split(rng)
        ant_rollout, (dones, rewards, _,  _, _) = rollout(_rng, env, env_params, ant_train_state, ActorCritic.initialize_carry((config["num_train_envs"],)), levels, config["student_num_steps"], "student_", config["num_train_envs"])
        ...
        pro_obs_densities = jax.lax.dynamic_slice(obs_densities, (0, 0, 0, 0), (config["num_train_envs"]//2, 21, 21, 1))
        ant_obs_densities = jax.lax.dynamic_slice(obs_densities, (config["num_train_envs"]//2, 0, 0, 0), (config["num_train_envs"]//2, 21, 21, 1))

        # Adversary Rewards
        pro_est_regret = jax.lax.dynamic_slice(ant_max_returns - pro_mean_returns, (0,), (config["num_train_envs"]//2,))
        ant_est_regret = jax.lax.dynamic_slice(pro_max_returns - ant_mean_returns, (config["num_train_envs"]//2,), (config["num_train_envs"]//2,))
        pro_adv_rollout = compute_adv_advantage(pro_adv_rollout, pro_est_regret, pro_adv_last_value, pro_obs_densities)
        ant_adv_rollout = compute_adv_advantage(ant_adv_rollout, ant_est_regret, ant_adv_last_value, ant_obs_densities)
        
                
        (rng, pro_train_state), pro_losses = update(rng, pro_train_state, ActorCritic.initialize_carry((config["num_train_envs"],)), pro_rollout, "student_", config["num_train_envs"])
        (rng, ant_train_state), ant_losses = update(rng, ant_train_state, ActorCritic.initialize_carry((config["num_train_envs"],)), ant_rollout, "student_", config["num_train_envs"])
        (rng, pro_adv_train_state), pro_adv_losses = update(rng, pro_adv_train_state, AdversaryActorCritic.initialize_carry((config["num_train_envs"]//2,)), pro_adv_rollout, "adv_", config["num_train_envs"]//2)
        (rng, ant_adv_train_state), ant_adv_losses = update(rng, ant_adv_train_state, AdversaryActorCritic.initialize_carry((config["num_train_envs"]//2,)), ant_adv_rollout, "adv_", config["num_train_envs"]//2)

        
        metrics = {
            "pro_losses": jax.tree.map(lambda x: x.mean(), pro_losses),
            "ant_losses": jax.tree.map(lambda x: x.mean(), ant_losses),
            "pro_adv_losses": jax.tree.map(lambda x: x.mean(), pro_adv_losses),
            "ant_adv_losses": jax.tree.map(lambda x: x.mean(), ant_adv_losses),
            ...
            "pro_est_regret":   pro_est_regret,
            "ant_est_regret":   ant_est_regret,
            "pro_eps": pro_eps,
            "ant_eps": ant_eps,
            "levels": levels,
        ...
        train_state = train_state.replace(
            ...
            pro_adv_train_state=pro_adv_train_state,
            ant_adv_train_state=ant_adv_train_state,
        )
