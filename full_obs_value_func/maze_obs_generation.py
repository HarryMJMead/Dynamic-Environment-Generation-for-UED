...
def main(config=None, project="JAXUED_TEST"):
    ...
    tags = ["obs_gen", config["metric"]]
    if config["sparse"]:
        tags.append("sparse")
    if config["critic_train_optimal"]:
        tags.append("critic_train_optimal")
    run = wandb.init(config=wandb_config, project=project, tags=tags)
    ...
    def log_eval(stats):
        ...
        log_dict = {
            ...
            "misc/negative_value_loss":   stats['negative_value_loss'].mean(),
            "misc/critic_negative_value_loss":   stats['critic_negative_value_loss'].mean(),
            "misc/critic_positive_value_loss":   stats['critic_positive_value_loss'].mean(),
            "misc/critic_advantage":   stats['critic_advantage'].mean(),
            "misc/optimal_advantage":   stats['optimal_advantage'].mean(),
            "misc/learnability":   stats['learnability'].mean(),
            "losses/pro_value_loss": stats['pro_losses'][1][0].mean(),
            "losses/crit_value_loss": stats['crit_losses'][1][0].mean(),
            "losses/crit_optimal_MSE": stats['crit_optimal_MSE'].mean(),
            "losses/pro_optimal_MSE": stats['pro_optimal_MSE'].mean(),
        }
    ...
    def create_train_state(rng):
        ...
        return TrainState(
            ...
            critic_train_state = create_inner_train_state(rng_crit, crit_env, adv_env_params, Critic, "critic_"),
    ...
    def train_step(carry, _):
        ...
        def update_critic(rng, train_state, init_hstate, rollout):
            ...
            return update_critic_rnn(
                ...
                config["critic_num_minibatches"],
                config["critic_epoch_ppo"],
                config["critic_clip_eps"],
                update_grad=True,
            )
        ...
        # Get Rollout for Full Observatin Critic
        critic_rollout, crit_advantage = get_critic_rollout(pro_traj, pro_carry, pro_locations)
        ...
        negative_value_loss = jnp.maximum(-pro_advantage, 0)

        # Get Critic NVL
        crit_advantage /= pro_eps
        crit_negative_value_loss = jnp.maximum(-crit_advantage, 0)
        crit_positive_value_loss = jnp.maximum(crit_advantage, 0)

        # Get Optimal Returns and Value Function MSE
        def get_optimal_return():
            ...
            return jnp.maximum((1 - 0.9 * (optimal_return_dist / config["student_num_steps"])), 0), jnp.maximum((1 - 0.9 * (last_optimal_return_dist / config["student_num_steps"])), 0)
        ...
        optimal_advantage /= pro_eps

        # Set Critic Target to optimal_return
        if config["critic_train_optimal"]:
            crit_obs, crit_dones, crit_values, _ = critic_rollout
            critic_rollout = crit_obs, crit_dones, crit_values, optimal_return

        # Select Adv Optimisation Metric
        if config["metric"] == "optimal_regret":
            optimisation_metric = agent_regret / config["student_num_steps"]
        elif config["metric"] == "PVL":
            optimisation_metric = positive_value_loss / config["student_num_steps"]
        elif config["metric"] == "NVL":
            optimisation_metric = negative_value_loss / config["student_num_steps"]
        elif config["metric"] == "learnability":
            optimisation_metric = learnability[None, :]
        elif config["metric"] == "crit_NVL":
            optimisation_metric = (crit_negative_value_loss) / config["student_num_steps"]
        elif config["metric"] == "crit_neg_adv":
            optimisation_metric = (-crit_advantage) / config["student_num_steps"] 
        elif config["metric"] == "opt_neg_adv":
            optimisation_metric = (-optimal_advantage) / config["student_num_steps"] 
        ...
        metrics = {
            ...
            "crit_losses": jax.tree_map(lambda x: x.mean(), critic_losses),
            "crit_optimal_MSE": crit_optimal_MSE,
            "pro_optimal_MSE": pro_optimal_MSE,
            ...
            "negative_value_loss": negative_value_loss.sum(axis=0),
            "learnability": learnability,
            "critic_negative_value_loss": crit_negative_value_loss.sum(axis=0),
            "critic_positive_value_loss": crit_positive_value_loss.sum(axis=0),
            "critic_advantage": crit_advantage.sum(axis=0),
            "optimal_advantage": optimal_advantage.sum(axis=0),
            "pro_eps": pro_eps,
            "levels": levels,
        }
