...
def main(config=None, project="JAXUED_TEST"):
    ...
    run = wandb.init(config=wandb_config, project=project, tags=["optimal_regret", "fully_obs"])
    ...
    def train_step(carry, _):
        ...
        # protagonist rollout
        rng, _rng = jax.random.split(rng)
        pro_rollout, (dones, rewards, last_pro_env_state, _, _) = rollout(_rng, env, env_params, pro_train_state, ActorCritic.initialize_carry((config["num_train_envs"],)), levels, config["student_num_steps"], "student_")
        pro_mean_returns, pro_max_returns, pro_eps = compute_max_mean_returns_epcount(dones, rewards)

        # get optimal regret
        pro_locations = pro_rollout[0].agent_location
        ...
        pro_locations = jnp.concatenate([pro_locations, last_agent_location[None, ...]], axis=0)

        # Get Optimal Distances
        agent_optimal_distances = jax.vmap(
            get_agent_min_steps_to_goal,
            in_axes=(0, 1)
        )(last_env_state, pro_locations).swapaxes(0, 1)

        # Compute one-step pro regret
        agent_regret = jnp.nan_to_num((1 + agent_optimal_distances[1:]) - (agent_optimal_distances[:-1]), 0) * (1 - dones) / pro_eps
        total_regret = agent_regret.sum(axis=0) / config["student_num_steps"]

        obs, actions, dones, log_probs, values, targets, advantages = adv_rollout

        # Adversary Rewards
        rewards = jnp.zeros_like(values).at[-1].set(total_regret)
